...
Vagrant.configure("2") do |config|
  ...
  config.vm.box = "devHowlingSails"  # Base Install with+ UI replace with https://linuxconfig.org/how-to-install-xubuntu-desktop-on-ubuntu-18-04-bionic-beaver-linux

  File.open("hostname.txt", "w") {|f| f.write("howlingstails-#{SecureRandom.random_number(9999)}")} unless File.file?("hostname.txt")
...
#               (.bash_profile loaded in terminal)
